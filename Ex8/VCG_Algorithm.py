...
def VCG_Algorithm(agents : List[Agent]):
    ...
    max,s = max_weight_matching_algorithm(G)
    print("max_option:", "=", max)
    #print(s)
    G1 = G.copy()
    for i in agents:
        ...
        #print(w)
        max_option = max - get_value_player(G,s,i.name)
        print(i.name, "pay: ", max_without_player ,"-",max_option, "=", max_without_player- (max - get_value_player(G,s,i.name)))
...
def max_weight_matching_algorithm(G):
    ...
    for node1, node2 in s:
        for i in G.get_edge_data(node1, node2).values():
            max += i

    return max,s

def get_value_player(G,s,option):
    #print(s)
    value = 0
    for node1, node2 in s:
